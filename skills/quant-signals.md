...
### Fondamentaux — SEC EDGAR d'abord (gratuit, sans clé, US), FMP en repli
- Source primaire des titres US : les **10-K XBRL officiels** de `data.sec.gov/api/xbrl/companyfacts`
  (champ `fundamentals_source: "SEC EDGAR (10-K)"` dans signals.json). C'est la donnée la plus
  fiable qui existe, et elle ne dépend d'aucun quota. FMP (free tier lacunaire) n'est plus
  qu'un repli, notamment pour les non-US.
...
- **Qualité des earnings** : `accruals_ratio = (résultat net − CFO) / actifs` et `FCF/RN`
  (FCF EDGAR = CFO − capex). Accruals élevés / FCF faible = bénéfices peu adossés au cash →
  drapeau `ambre`/`rouge`.
- Les européens/HK ne sont pas chez EDGAR : F-Score `null` (data_gap), le gate conclut sur
  les signaux de prix — c'est voulu, pas un bug.
...
## Outils annexes du moteur

- **`node engine/bench.js {opened} {closed}`** — rendement du **MSCI World EUR** (IWDA.AS,
  Yahoo, sans clé) sur la période d'une position clôturée. Sert à la passe d'apprentissage du
  vendredi : `alpha_pct = realized_pnl_pct − benchmark_return_pct`. **L'alpha mesure le skill,
  le P&L brut mesure surtout le marché** (method §I).
- **`memory/run-report.json`** — journal AUTOMATIQUE des runs, écrit par `engine/push-memory.js`
  à chaque persistance (routine, date, fichiers). Un trou dans les nuits Lun-Ven = une routine
  qui a planté avant de persister : le vendredi le signale dans la revue hebdo.
