...
//   4) pré-historique (< inception) purgé (on repart propre → relancer backfill = historique figé, puis value).
...
export async function GET(request: NextRequest) {
  ...
  // ── 4) Purge le pré-historique SANS toucher au vrai track record : on n'efface que les
  // snapshots ANTÉRIEURS à l'inception (la courbe reconstruite). Les vrais relevés quotidiens
  // (≥ inception, écrits par value) sont préservés → bootstrap reste rejouable sans perdre
  // l'historique réel. Relancer ensuite « Historique figé » (backfill = clone d'origine
  // reconstruit avant l'inception) puis « Valoriser le jour » (value).
  ...
  return NextResponse.json({
    ...
    next: "Lancer /api/cron/backfill (historique figé) puis /api/cron/value.",
